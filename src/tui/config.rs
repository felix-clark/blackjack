...
use super::index::{INDEX_TAIL_MASS, banded_crossing, occurrence_window};
...
impl ShoeChoice {
    ...
    /// The lowest count at which taking insurance turns positive-EV under `system` — the insurance "key
    /// count" (running count for KO, integer true count for Hi-Lo), or `None` on the infinite deck or if
    /// no realistically reachable count makes it pay. Independent of the player's current count value (it
    ...
    pub(super) fn insurance_key_count(self, system: CountSystemId) -> Option<i16> {
        let ShoeChoice::Decks(n) = self else {
            return None;
        };
        let up = Card::Ace;
        let pen = COUNT_PENETRATION;
...
mod tests {
    ...
    fn insurance_key_counts_match_published_indices() {
        assert_eq!(
            ShoeChoice::Decks(1).insurance_key_count(CountSystemId::Ko),
            Some(3),
            "single-deck KO insurance index is +3"
        );
        assert_eq!(
